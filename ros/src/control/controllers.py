...
class SteeringAngleController():
    ...
    def control(self, current_linear_velocity, target_linear_velocity, target_angular_velocity):
        ...
        # Adjust target_angular_velocity based on target_linear_velocity
        # Ensures proportional movement when linear velocity is non-zero
        ...
        # Limit target_angular_velocity based on current_linear_velocity
        # Prevents excessive yaw rates at low linear velocities to maintain stability
        if abs(current_linear_velocity) > 0.1:
            max_yaw_rate = abs(self.max_lateral_acc / current_linear_velocity)
            target_angular_velocity = max(-max_yaw_rate, min(max_yaw_rate, target_angular_velocity))
